...
using JebsBoxOfTears.Unity.Runtime;
using UitkForKsp2.API;
using UnityEngine;
using UnityEngine.UIElements;

namespace JebsBoxOfTears.UI;
...
public class MyFirstWindowController : MonoBehaviour
{
    ...
    public bool IsWindowOpen
    {
        ...
        set
        {
            ...
            GameObject.Find(JebsBoxOfTearsPlugin.ToolbarFlightButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);

            // Update the OAB AppBar button state
            GameObject.Find(JebsBoxOfTearsPlugin.ToolbarOabButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);
        }
    }
